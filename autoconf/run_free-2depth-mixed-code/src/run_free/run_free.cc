...
extern void freeHistoryList(GSList *h);
...
{
  /* performes a system call like system.  This is the recomended
...
void combo_keyPress(GtkWidget *widget, GdkEventKey *event, gpointer *data)
{
  ...
  */
  ...
  //  else if(event->keyval == GDK_Tab) // command line completion
     // {
      //      if(prevKey == GDK_Control_L || prevKey == GDK_Control_R)
      //find_history_item();
     // }
...
void runClicked(GtkWidget *widget, gpointer data)
{
  ...
  */
  ...
  //get the entry field from the combo and set it to a local
...
void browseClicked(GtkWidget *widget, gpointer data)
{
  ...
  gtk_signal_connect_object( GTK_OBJECT (GTK_FILE_SELECTION(fDialog)->cancel_button),
			     "clicked",
			     /*GTK_SIGNAL_FUNC(fileDialog_keyPress),*/
...
int main(int argc, char **argv, char **e)
{
  ...
  *run_in_term = 0;
  ...
  //################## group table
  /*
    NOTE: I picked the number of table elements at random.  I wasn't
    sure how this would effect the look of this app on other hardware,
    etc. either.  
    This table is set inside the dfault table within the dialog box's
    label/text area.
  */
  ...
  cbitems = g_list_append(cbitems, ""); //first item (shows) blank
